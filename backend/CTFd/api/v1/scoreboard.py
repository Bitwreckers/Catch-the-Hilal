...
from CTFd.models import Brackets, Solves, Users, db
...
class ScoreboardList(Resource):
    ...
    def get(self):
        ...
        # Solves count and last submission time per account (respects freeze)
        ...
        if mode == TEAMS_MODE:
            solve_stats_q = (
                db.session.query(
                    ...
                    func.max(Solves.date).label("last_date"),
                )
        ...
        if freeze:
            solve_stats_q = solve_stats_q.filter(
                Solves.date < unix_time_to_utc(freeze)
            )
        solve_stats = {
            row.account_id: {
                "solves": row.solves,
                "last_date": row.last_date.isoformat() + "Z"
                if row.last_date
                else None,
            }
            for row in solve_stats_q.all()
        }
        ...
        for i, x in enumerate(standings):
            ...
            stats = solve_stats.get(x.account_id, {})
            entry["solves"] = stats.get("solves", 0)
            entry["last_submit_time"] = stats.get("last_date")
