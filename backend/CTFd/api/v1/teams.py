...
class TeamPublic(Resource):
    ...
    def get(self, team_id):
        ...
        # Query members by team_id so list is always correct (relationship may not load)
        member_list = Users.query.filter_by(team_id=team.id).all()
        response.data["members"] = [{"id": m.id, "name": m.name} for m in member_list]
        return {"success": True, "data": response.data}
...
class TeamPrivate(Resource):
    ...
    def get(self):
        ...
        # Ensure members are always returned as list of {id, name} for the team page
        response.data["members"] = [
            {"id": m.id, "name": m.name} for m in team.members
        ]
        return {"success": True, "data": response.data}
...
class TeamPrivateMembers(Resource):
    ...
    @require_team
    def get(self):
        """Return list of {id, name} for current team members (for team page)."""
        team = get_current_team()
        # Query members directly from DB (team_id = team.id) so list is always correct
        member_list = Users.query.filter_by(team_id=team.id).all()
        members = [{"id": m.id, "name": m.name} for m in member_list]
