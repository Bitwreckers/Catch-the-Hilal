...
def _login_credentials():
    """Get name and password from JSON or form (SPA or form login)."""
    if request.content_type and "application/json" in request.content_type:
        data = request.get_json(silent=True) or {}
        name = (data.get("name") or data.get("username") or "").strip()
        password = data.get("password") or ""
        return name, password
    return (request.form.get("name") or "").strip(), request.form.get("password") or ""
...
def login():
    errors = get_errors()
    if request.method == "POST":
        name, password = _login_credentials()
            ...
            if (
                ...
                if admin:
                    ...
                    if request.is_json:
                        return {"success": True}
                    return redirect(url_for("challenges.listing"))
                # Preset user exists but is not admin (e.g. created by create_admin.py); fall through to normal login
        ...
        if user:
            ...
            if user and verify_password(password, user.password):
                ...
                db.session.close()
                if request.is_json:
                    return {"success": False, "error": "Your username or password is incorrect"}, 401
                return render_template("login.html", errors=errors)
        else:
